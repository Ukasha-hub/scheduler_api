...
def create_filter(payload: FilterCreate, emp_id: str = Query(...), db: Session = Depends(get_db)):
    def operation():
        db_obj = Filter(type=payload.type, color=payload.color)
        db.add(db_obj)
        db.flush()
        log_action(db, emp_id=emp_id, action=f"Created filter: {payload.type}, {payload.color}")
        return db_obj
...
def delete_filter(filter_id: int, emp_id: str = Query(...), db: Session = Depends(get_db)):
    def operation():
        ...
        db.delete(db_obj)
        db.flush()
        log_action(db, emp_id=emp_id, action=f"Deleted filter: type:{db_obj.type}, color:{db_obj.color}")
...
def update_filter(filter_id: int, payload: FilterUpdate, emp_id: str = Query(...), db: Session = Depends(get_db)):
    def operation():
        db_obj = db.query(Filter).filter(Filter.id == filter_id).first()
        if not db_obj:
        ...
        db.flush()
        log_action(db, emp_id=emp_id, action=f"Updated filter to: type:{payload.type}, color:{payload.color}")
        return db_obj
